...
arg_parser = argparse.ArgumentParser(description='') # TODO description
...
class JsonToText:
    ...
    def __init__(self):
        self.unresolved_template_names = {}

    def resolve_templates(self, data):
        output_string = ''
        for item in data:
                ...
                if 'tmpl' in item:
                    ...
                    for template_item in item['tmpl']:
                        if is_first:
                            ...
                            self.unresolved_template_names[template_item[0]] = None
                            print('UNRESOLVED: {}'.format(template_item[0]))
                            output_string += '<span style="color: #99f">{}</span>'.format(self.resolve_templates(template_item)) # TODO: really need to call resolve_templates here?
                        else:
                            output_string += '<span style="color: red">, </span>'
                        # output_string += escape(str(template_item))
                            output_string += self.resolve_templates(template_item)
                    output_string += '<span style="color: red">)</span>'
                ...
                for book in data['body']:
                    ...
                    for hebrew_chapter_number in chapters:
                        ...
                        for hebrew_verse_number in chapter:
                            ...
                            resolved_html = self.resolve_templates(verse[2])
                            if resolved_html:
                                output_file.write('<h3>{}</h3>'.format(escape(hebrew_verse_number)))
                                output_string = '\n<p>'
                                output_file.write(resolved_html)
                                output_string += '</p>'
